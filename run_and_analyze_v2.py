...
# 디렉토리 설정
base_dir = os.getcwd()  # Jupyter에서는 __file__ 대신 현재 경로 사용
log_dir = os.path.join(base_dir, "logs_0807")
...
def run_gpu_burn(index):
    subprocess.run(["bash", "-c", f"CUDA_VISIBLE_DEVICES={index} /home/kyumin/gpu-test/gpu-burn/gpu_burn 3600"], cwd="/home/kyumin/gpu-test/gpu-burn")
...
def run_stress_ng():
    with open(stress_log_path, "w") as f:
        subprocess.run([
            ...
            "--timeout", "3600s", "--metrics-brief"
        ], stdout=f, stderr=f)

# -------------------------- 주기적 시각화 스레드 --------------------------
def periodic_visualization(interval_sec=30, duration_sec=3600):
    iterations = duration_sec // interval_sec
    for i in range(iterations):
        elapsed = i * interval_sec
        print(f"📊 시각화 실행 (경과 {elapsed}초)")
        try:
...
visual_thread = threading.Thread(target=periodic_visualization, args=(30, 3600))
visual_thread.start()
